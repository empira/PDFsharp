...
namespace PdfSharp.TestHelper.Analysis.ContentStream
{
    ...
    /// </summary>
    public class GetText : GetObjectBase<GetText.TextInfo>
    {
        public GetText(ContentStreamEnumerator contentStreamEnumerator) : base(contentStreamEnumerator)
        { }
        ...
        {
            var tjState = _contentStreamEnumerator.GetState();

            // Find prepending string object.
            if (!_contentStreamEnumerator.MovePrevious())
                return GetObjectFailed();
            var text = _contentStreamEnumerator.Current;
            var textState = _contentStreamEnumerator.GetState();
            ...
            out ContentStreamEnumerator.IState? tdXState, out ContentStreamEnumerator.IState? tdYState, out ContentStreamEnumerator.IState? tdState)
        {
            ...
            if (!MoveToPreviousTdInTextObject())
                return false;
            tdState = _contentStreamEnumerator.GetState();
            ...
            out ContentStreamEnumerator.IState? tdXState, out ContentStreamEnumerator.IState? tdYState)
        {
            ...
            if (!_contentStreamEnumerator.MovePrevious())
                return false;
            var yOffsetStr = _contentStreamEnumerator.Current;
            if (!PdfFileHelper.TryParseDouble(yOffsetStr, out yOffset))
                return false;
            tdXState = _contentStreamEnumerator.GetState();

            // Find prepending x offset.
            if (!_contentStreamEnumerator.MovePrevious())
                return false;
            var xOffsetStr = _contentStreamEnumerator.Current;
            if (!PdfFileHelper.TryParseDouble(xOffsetStr, out xOffset))
                return false;
            tdYState = _contentStreamEnumerator.GetState();
        ...
        bool MoveToPreviousTdInTextObject()
        {
            while (true)
            {
                ...
                if (_contentStreamEnumerator.Current == "Td")
                    return true;
            }
        }
