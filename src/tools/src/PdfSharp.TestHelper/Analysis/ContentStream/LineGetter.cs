...
namespace PdfSharp.TestHelper.Analysis.ContentStream
{
    ...
    /// </summary>
    public class LineGetter : ObjectGetterBase<LineGetter.LineInfo>
    {
        public LineGetter(ContentStreamEnumerator contentStreamEnumerator) : base(contentStreamEnumerator)
        { }
        ...
        {
            var lState = ContentStreamEnumerator.GetState();

            // Y of the l element is y2.
            if (!ContentStreamEnumerator.MovePrevious())
                return GetObjectFailed();
            var y2Str = ContentStreamEnumerator.Current!;

            // X of the l element is x2.
            if (!ContentStreamEnumerator.MovePrevious())
                return GetObjectFailed();
            var x2Str = ContentStreamEnumerator.Current!;

            // Find the previous m or l element.
            if (!ContentStreamEnumerator.MovePrevious())
                return GetObjectFailed();
            if (ContentStreamEnumerator.Current is not ("m" or "l"))
                return GetObjectFailed();

            // Y of the previous m/l element is y1.
            if (!ContentStreamEnumerator.MovePrevious())
                return GetObjectFailed();
            var y1Str = ContentStreamEnumerator.Current!;

            // X of the previous m/l element is x1.
            if (!ContentStreamEnumerator.MovePrevious())
                return GetObjectFailed();
            var x1Str = ContentStreamEnumerator.Current!;

            // Move back to the identifying l element.
            ContentStreamEnumerator.SetState(lState);
