...
namespace PdfSharp.Drawing
{
    ...
    static class GeometryHelper
    {
        ...
        public static PathGeometry CreatePolygonGeometry(SysPoint[] points, XFillMode fillMode, bool closed)
        {
            ...
            {
                FillRule = fillMode == XFillMode.Winding ? FillRule.Nonzero : FillRule.EvenOdd
            };
            geo.Figures.Add(fig);
            return geo;
        }
          ...
          double sweepAngle, out SysPoint startPoint)
        {
            ...
            ArcSegment seg = new ArcSegment(destPoint, size, 0, isLargeArc, sweepDirection, true);
            return seg;
        }
#endif
        ...
        // to a series of Bézier curves exactly the way WPF renders the arc. I tested my own code
        ...
        * 
            ...
            bool isLargeArc, bool isClockwise, double xEnd, double yEnd, out int pieces)
        {
            ...
            GetArcAngle(ptStart, ptEnd, isLargeArc, isClockwise, out var cosArcAngle, out var sinArcAngle, out pieces);

            // Get the vector to the first Bezier control point 
            var bezDist = GetBezierDistance(cosArcAngle, 1);
        ...
        static bool AcceptRadius(double halfChord2, double fuzz2, ref double radius)
        {
            ...
            var accept = radius * radius > halfChord2 * fuzz2;
            if (accept)
            {
                if (radius < 0)
                    radius = 0;
