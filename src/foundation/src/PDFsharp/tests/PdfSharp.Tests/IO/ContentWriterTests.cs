...
namespace PdfSharp.Tests.IO
{
    ...
    public class ContentWriterTests : IDisposable
    {
        ...
        public void Read_Write_Compare_Sample_Files(string file0)
        {
            ...
            doc2.Save(filename2);
            //// … and start a viewer.
            //PdfFileUtility.ShowDocumentIfDebugging(filename2);
            ...
            var filename3 = filename.Replace(".pdf", "_compare.pdf");
            // Save the document…
        ...
        public void Create_simple_PDF()
        {
            ...
            // This is no real operator, just something that looks somewhat real. Like a "Lorem ipsum" text.
        ...
        public void Modify_HelloWorld()
        {
            ...
            foreach (var item in contents)
            {
                ...
                {
                    //var newOp = @operator.Clone();
                    if (@operator.OpCode.OpCodeName == OpCodeName.Tj)
                    {
                        //var newOps = new CSequence();
                        foreach (var op in @operator.Operands)
                        {
                            if (op is CString @string)
                            {
                                if (idx == 0)
                                    @string.Value = "Hero";
                                if (idx == 1)
                                    @string.Value = "Word!!! ABCXYZ abcxyz ÄÖÜäöüß";
                                ++idx;
                            }
                        }
                        //@operator.Operands = newOps;
                        newContent.Add(item);
                    }
                    else
                    {
